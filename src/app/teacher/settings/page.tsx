  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-tighter uppercase mb-1 italic">Command & Preference</h1>
          <p className="text-muted-foreground font-medium text-sm">Manage your teacher credentials, signature, and availability.</p>
        </div>
        <Button 
          onClick={handleSaveProfile}
          disabled={isSaving}
          className="h-12 px-8 rounded-2xl bg-zinc-900 border-none shadow-2xl hover:bg-black text-white font-bold gap-2"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
         <div className="lg:col-span-1 space-y-6">
            <Card className="border-none shadow-xl bg-white dark:bg-zinc-900 rounded-3xl overflow-hidden pt-12">
               <div className="flex flex-col items-center">
                  ...
                  <div className="text-center mt-6 p-6">
                     <h3 className="font-extrabold text-lg italic">{profile?.name}</h3>
               ...
               <div className="border-t dark:border-zinc-800 p-2">
                  <Button variant="ghost" className="w-full justify-start gap-3 rounded-2xl font-bold h-12 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 uppercase text-[10px] tracking-widest">
            ...
            <Card className="border-none shadow-xl bg-zinc-900 text-white p-6 rounded-3xl">
               <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-6 italic">Security Integrity</h3>
               <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                     ...
                     <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-tight">ENCRYPTED SESSION</p>
                  </div>
               </div>
               <p className="text-[10px] text-zinc-500 font-medium leading-relaxed uppercase tracking-widest">
                  Neural data paths secured via 256-bit AES protocol.
               </p>
            </Card>
         </div>
         ...
         <div className="lg:col-span-3">
            <Tabs defaultValue="info" className="space-y-6">
               <TabsList className="bg-white dark:bg-zinc-900 p-1.5 rounded-2xl shadow-xl h-auto border dark:border-zinc-800">
                  <TabsTrigger value="info" className="rounded-xl px-6 py-3 font-bold text-xs uppercase tracking-tighter data-[state=active]:bg-primary data-[state=active]:text-white italic">Professional Info</TabsTrigger>
                  <TabsTrigger value="signature" className="rounded-xl px-6 py-3 font-bold text-xs uppercase tracking-tighter data-[state=active]:bg-primary data-[state=active]:text-white italic">E-Signature</TabsTrigger>
                  <TabsTrigger value="notifications" className="rounded-xl px-6 py-3 font-bold text-xs uppercase tracking-tighter data-[state=active]:bg-primary data-[state=active]:text-white italic">Neural Alerts</TabsTrigger>
               </TabsList>
                  ...
                  <Card className="border-none shadow-2xl rounded-3xl overflow-hidden bg-white dark:bg-zinc-950 p-8">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-1 italic">Full Identity Name</label>
                           <Input 
                             value={profile?.name || ""} 
                             onChange={(e) => setProfile(prev => prev ? {...prev, name: e.target.value} : null)}
                             className="h-14 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none font-bold italic" 
                           />
                        </div>
                        <div className="space-y-2">
                           <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-1 italic">Institutional Email</label>
                           <div className="relative">
                              <Mail className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                              <Input value={profile?.email || ""} className="h-14 pl-11 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none font-bold opacity-60" disabled />
                           </div>
                        </div>
                        <div className="space-y-2">
                           <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-1 italic">Faculty Specialization</label>
                           <Input 
                        ...
                        <div className="space-y-2">
                           <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-1 italic">Primary Relay (Phone)</label>
                           <Input 
                             value={profile?.phone || ""} 
                        ...
                        <div className="md:col-span-2 space-y-2">
                           <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-1 italic">Degrees & Credentials</label>
                           <textarea 
                             value={profile?.bio || ""}
                             onChange={(e) => setProfile(prev => prev ? {...prev, bio: e.target.value} : null)}
                             className="w-full h-32 p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none font-medium text-sm outline-none focus:ring-2 ring-primary/20 resize-none font-bold"
                           />
                        </div>
                     </div>
                  </Card>
               ...
               <TabsContent value="signature">
                  <Card className="border-none shadow-2xl rounded-3xl overflow-hidden bg-white dark:bg-zinc-950 p-8">
                     <h3 className="text-xl font-black mb-2 italic">Institutional Signature</h3>
                     <p className="text-muted-foreground text-sm mb-8 font-medium">This signature will appear on all digital reports and student certifications.</p>
               ...
               <TabsContent value="notifications">
                  <Card className="border-none shadow-2xl rounded-3xl overflow-hidden bg-white dark:bg-zinc-950 p-10">
                     <h3 className="text-2xl font-black italic uppercase italic tracking-tighter mb-8 px-4">Neural Relay Settings</h3>
