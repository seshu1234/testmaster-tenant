...
export default function ManualGradingPage() {
  ...
  if (loading) return <div className="p-8 text-zinc-600 animate-pulse">Loading items needing review...</div>;

  if (responses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4 text-zinc-600">
        <CheckCircle2 className="h-16 w-16 text-zinc-600" />
        <h2 className="text-xl font-bold">Inbox Zero!</h2>
        <p className="text-zinc-600">All subjective answers for this test have been graded.</p>
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20 p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-zinc-600 font-black tracking-tight">Grading Console</h1>
            <p className="text-zinc-600 font-medium">Item {currentIndex + 1} of {responses.length} Pending</p>
          </div>
        </div>
        <Badge variant="outline" className="px-4 py-1.5 rounded-full border-primary/20 text-zinc-600 bg-primary/5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <Card className="rounded-3xl border shadow-xl bg-white overflow-hidden">
            <CardHeader className="bg-zinc-50 border-b p-6">
               <CardTitle className="text-xl font-bold">The Question</CardTitle>
            ...
            <CardContent className="p-8 space-y-8">
               <div className="space-y-3">
                  ...
                  <div className="p-6 bg-zinc-50 rounded-2xl border-2 border-dashed border-zinc-200 font-medium text-zinc-600 leading-loose">
               ...
               {current.model_answer && (
                 <div className="space-y-3 opacity-60 hover:opacity-100 transition-opacity">
                    ...
                    <div className="p-4 bg-emerald-50/50 rounded-xl border border-emerald-100 text-zinc-600">
                       {current.model_answer}
                    </div>
                 </div>
               )}
        ...
        <div className="space-y-6">
           <Card className="rounded-3xl border shadow-xl bg-zinc-900 text-white p-6 sticky top-24">
              <div className="space-y-6">
                 <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600 block mb-3">Award Marks (Max: {current.max_marks})</label>
                    <div className="flex items-end gap-3">
                       <input 
                         type="number" 
                         step="0.5"
                         max={current.max_marks}
                         className="bg-transparent border-b-2 border-zinc-700 text-zinc-600 font-black focus:border-primary outline-none py-2 w-24 text-zinc-600"
                         placeholder="0.0"
                         value={currentMarks}
                 ...
                 <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600 block mb-3">Feedback (Optional)</label>
                    <Textarea 
                      placeholder="Add a constructive note..." 
                      className="bg-zinc-800 border-none rounded-2xl min-h-[100px] text-zinc-600 focus:ring-2 ring-primary/20"
                 ...
                 <div className="pt-4 flex flex-col gap-3">
                    <Button 
                      className="w-full h-12 rounded-2xl font-bold gap-2 text-zinc-600 transition-all hover:scale-[1.02] active:scale-[0.98]"
                 ...
                 <div className="pt-6 border-t border-zinc-800 space-y-4 font-inter">
                    <div className="flex items-center gap-2 p-3 bg-zinc-800/50 rounded-xl border border-zinc-800 cursor-pointer hover:border-primary/50 transition-colors group">
                       <Sparkles className="h-4 w-4 text-zinc-600 group-hover:rotate-12 transition-transform" />
