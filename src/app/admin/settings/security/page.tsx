  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <Button variant="outline" className="gap-2 border-red-200 text-zinc-600 hover:bg-red-50">
          ...
          <CardContent>
            <div className="space-y-4">
               {data?.active_sessions.map((session, i) => (
                 <div key={i} className="flex items-center justify-between">
                   <div className="flex flex-col">
                     <span className="text-zinc-600 font-medium">{session.name} ({session.device || 'Unknown Device'})</span>
                     <span className="text-[10px] text-zinc-600 font-mono">
                       {session.email} • {session.last_used_at ? new Date(session.last_used_at).toLocaleString() : 'Active Now'}
                     </span>
                   </div>
                   {i === 0 ? (
                     <Badge variant="outline" className="text-zinc-600 border-green-200">Current</Badge>
                   ) : (
               ...
               {(!data?.active_sessions || data.active_sessions.length === 0) && (
                  <p className="text-zinc-600">No active sessions detected.</p>
               )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
             <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-100">
                  <div className="text-xl font-bold">{data?.audit_logs.length || 0}</div>
                  <p className="text-[10px] text-zinc-600 uppercase font-bold">Recent Actions</p>
                </div>
                <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-100">
                  <div className="text-xl font-bold text-zinc-600">Secure</div>
                  <p className="text-[10px] text-zinc-600 uppercase font-bold">System Status</p>
                </div>
             </div>
          </CardContent>
        </Card>
      ...
      <Card className="border shadow-sm bg-white/50 backdrop-blur-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="relative w-64">
             ...
             <Input placeholder="Search user or action..." className="pl-9 h-8 text-zinc-600" />
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-zinc-600">IP Address</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-zinc-600 py-8 animate-pulse text-zinc-600 uppercase font-bold tracking-widest">
                ...
                data?.audit_logs.map((log) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="font-medium text-zinc-600">{log.user?.name || 'System'}</TableCell>
                    <TableCell>
                       <Badge variant="outline" className="text-[9px] font-bold uppercase tracking-tight">
                        {log.action}
                       </Badge>
                    </TableCell>
                    <TableCell className="text-zinc-600">{log.entity_type}</TableCell>
                    <TableCell className="text-zinc-600 text-[10px] font-mono text-zinc-600">
                      {log.ip_address || '---'}
                    </TableCell>
                  </TableRow>
                ))
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent className="space-y-4">
             <div className="flex items-center justify-between">
               <div className="space-y-0.5">
                 <p className="text-xl font-bold">Require 2FA</p>
                 <p className="text-[10px] text-zinc-600">Force multi-factor for all admins.</p>
               </div>
               <Switch />
             </div>
             <div className="flex items-center justify-between">
               <div className="space-y-0.5">
                 <p className="text-xl font-bold">Session Expiry</p>
                 <p className="text-[10px] text-zinc-600">Lock interface after 30 mins idle.</p>
               </div>
        ...
        <Card className="border shadow-sm bg-white/50 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
             <Button variant="outline" className="w-full h-10 text-zinc-600 gap-2">
