...
export default function FamilyDashboardPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20 p-6">
      <div>
         <h1 className="text-zinc-600 font-black tracking-tighter  uppercase">Family Intelligence Matrix</h1>
         <p className="text-zinc-600 font-medium">Side-by-side performance telemetry for multi-ward synchronization.</p>
      </div>

      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <div className="h-64 bg-zinc-100 rounded-[3rem] animate-pulse" />
           <div className="h-64 bg-zinc-100 rounded-[3rem] animate-pulse" />
        </div>
      ) : (
              ...
              <Card key={child.id} className={cn(
                 ...
                 <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
                    ...
                    <div className="flex-1 text-zinc-600 md:te">
                       ...
                       <h2 className="text-zinc-600 font-black  uppercase  tracking-tighter leading-none mb-4">{child.name}</h2>
                       <div className="grid grid-cols-3 gap-6">
                          <div>
                             <p className="text-[8px] font-black uppercase text-zinc-600 mb-1">Avg Score</p>
                             <p className="text-zinc-600 font-black ">{child.avgScore}%</p>
                          </div>
                          <div>
                             <p className="text-[8px] font-black uppercase text-zinc-600 mb-1">Streak</p>
                             <p className="text-zinc-600 font-black  text-zinc-600">{child.streak}🔥</p>
                          </div>
                          <div>
                             <p className="text-[8px] font-black uppercase text-zinc-600 mb-1">Rank</p>
                             <p className="text-zinc-600 font-black ">{child.rank}</p>
                          </div>
                       </div>
                    </div>
                 </div>
                 {i === 0 && <Users className="absolute -bottom-10 -right-10 h-48 w-48 text-zinc-600 opacity-5 rotate-12" />}
              </Card>
           ))}
        </div>
      )}
      ...
      <Card className="border-none shadow-2xl rounded-[3rem] bg-white p-12">
         <div className="flex justify-between items-center mb-12">
            <div>
               <h3 className="text-zinc-600 font-black  uppercase  tracking-tighter">Correlation Analysis</h3>
               <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600 mt-1">CROSS-WARD COMPETENCY MAPPING</p>
            </div>
            <div className="flex items-center gap-6">
               ...
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">{children[1]?.name}</span>
               </div>
            </div>
         </div>
           ...
           { label: 'Global Ranking', value: 'Top 2%', icon: Zap, color: 'te' }
            ...
            <Card key={i} className="border-none shadow-xl rounded-[2.5rem] bg-white p-8 flex items-center justify-between group hover:border-primary transition-all">
               <div>
                  ...
                  <p className="text-zinc-600 font-black  uppercase  tracking-tighter">{stat.value}</p>
               </div>
