  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-zinc-600 font-black tracking-tighter  uppercase">Tactical Schedule</h1>
          <p className="text-zinc-600 font-medium">Monitoring upcoming assessments and study milestones.</p>
        </div>
           ...
           <div className="space-y-4">
              {isLoading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="h-40 bg-zinc-100 rounded-[2.5rem] animate-pulse" />
                ))
              ) : tests.length > 0 ? (
                tests.map((test) => (
                  <Card key={test.id} className="border-none shadow-xl rounded-[2.5rem] bg-white overflow-hidden group hover:scale-[1.01] transition-all">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                           <div className="flex items-center gap-8">
                              <div className="h-24 w-24 rounded-[2rem] bg-zinc-950 flex flex-col items-center justify-center shrink-0 shadow-2xl relative overflow-hidden group-hover:bg-primary transition-colors">
                                 <span className="text-[10px] font-black uppercase text-zinc-600 group-hover:te/60">{new Date(test.startTime).toLocaleDateString('en-US', { month: 'short' })}</span>
                                 <span className="text-zinc-600 font-black  text-zinc-600 leading-none whitespace-nowrap">{new Date(test.startTime).getDate()}</span>
                                 <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                              </div>
                              
                              <div className="space-y-2">
                                 <div className="flex items-center gap-3">
                                    <Badge variant="outline" className="text-[9px] font-black uppercase px-3 h-6 bg-zinc-50 border-zinc-200">{test.subject}</Badge>
                                    <Badge className="bg-emerald-500/10 text-zinc-600 border-none text-[9px] font-black uppercase px-3 h-6">
                                       {test.type}
                                    </Badge>
                                 </div>
                                 <h4 className="text-zinc-600 font-black  uppercase  tracking-tighter leading-tight">{test.title}</h4>
                                 <div className="flex items-center gap-6 text-[10px] font-black text-zinc-600 uppercase tracking-widest mt-2">
                           ...
                           <div className="flex items-center gap-4 w-full md:w-auto">
                              <Button className="flex-1 md:flex-none h-14 px-8 rounded-2xl bg-zinc-900 text-white font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all">
                                 SET SMS REMINDER
                              </Button>
                              <Button size="icon" variant="ghost" className="h-14 w-14 rounded-2xl bg-zinc-50 hover:bg-black  transition-all">
                ...
                <div className="py-20 text-zinc-600 bg-white rounded-[3rem] border-2 border-dashed border-zinc-100">
                   <CalendarIcon className="h-12 w-12 text-zinc-600 mx-auto mb-4" />
                   <p className="text-zinc-600 font-black uppercase  tracking-widest text-zinc-600">No imminent assessments detected.</p>
                </div>
              )}
           </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           ...
           <Card className="border-none shadow-2xl rounded-[3rem] bg-zinc-950 text-zinc-600 p-10">
              <div className="flex items-center gap-4 mb-10">
                 ...
                 <div>
                    <h4 className="text-zinc-600 font-black  uppercase  tracking-tighter">System Health</h4>
                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600">READINESS VERIFIED</p>
                 </div>
              </div>
           ...
           <Card className="border-none shadow-xl rounded-[2.5rem] bg-rose-50 p-10 border border-rose-100">
